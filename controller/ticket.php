...
switch ($_GET['op']) {
    case "insert":
        $datos = $ticket->insert_ticket($_POST["usu_id"], $_POST["cat_id"], $_POST["tick_titulo"], $_POST["tick_descrip"], $_POST["prio_id"]);
    ...
    case "listar_x_usu":
        ...
        foreach ($datos as $row) {
            ...
            $sub_array[] = '<button type="button" onClick="ver(' . $row["tick_id"] . ')" id="' . $row["tick_id"] . '" class="btn btn-inline-primary btn-sm ladda-button"><i class="fa fa-eye"></i></button>';
            $data[] = $sub_array;
        }
...
<?php
    ...
    case "update":
        $ticket->updateTicket($_POST["tick_id"]);
        $ticket->insert_ticketdetalle_cerrar($_POST["tick_id"], $_POST["usu_id"]);
        break;

        case "reabrir":
            $ticket->reabrirTicket($_POST["tick_id"]);
